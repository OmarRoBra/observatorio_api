...
export default class HolidayStats extends Model {
  @Column({ field: 'year', type: DataType.INTEGER }) year!: number;
  @Column({ field: 'bridge_name', type: DataType.STRING }) bridge_name!: string;
  @Column({ field: 'municipality', type: DataType.STRING }) municipality!: string;
  ...
  @Column({ field: 'nights', type: DataType.INTEGER }) nights!: number;
  ...
  // Timestamps (opcional, pero recomendado si usas createdAt/updatedAt)
